    ...
    const handleDrawCreate = (e: any) => {
      ...
      if (geojson.geometry.type === 'Polygon') {
        ...
        center = turf.centerOfMass(
          turf.multiPolygon(geojson.geometry.coordinates)
        ).geometry.coordinates
      }

      // Convert center to pixel coordinates
      const pixelPosition = center
        ? map.project(center as [number, number])
        : null

      // Set the shape property for regular polygons
      if (!geojson.properties) geojson.properties = {}
      geojson.properties.shape = geojson.properties.shape
        ? geojson.properties.shape
        : 'polygon'
