...
export default function BasedOnLayerDropdown({ layerIndex }: BasedOnLayerDropdownProps) {
  const { basedOnLayerId, setBasedOnLayerId, geoPoints, basedOnProperty, setBasedOnProperty } = useCatalogContext();
  ...
  return (
    ...
    <div className="ms-2.5 flex flex-col">
      <label htmlFor="basedOnPropertyDropdown" className="text-[11px] my-[2px] text-[#555] whitespace-nowrap text-sm">
        Based on
      </label>
      <select
      ...
      >
        ...
        {
          metrics.map(metric => {
            return <option key={metric} value={metric}>{formatSubcategoryName(metric)}</option>
          })
        }
      </select>
    </div>
